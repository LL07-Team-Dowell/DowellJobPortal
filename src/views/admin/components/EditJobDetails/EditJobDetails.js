...
const EditJobDetails = ({ currentJob, currentJobState, updateJobDetails }) => {
    ...
    useEffect(() => {
        ...
        if (currentJob.others && currentJob.others[jobKeys.paymentForJob]) allOtherTerms[jobKeys.paymentForJob] = currentJob.others[jobKeys.paymentForJob]
        if (currentJob.others && currentJob.others[jobKeys.othersInternJobType]) allOtherTerms[jobKeys.othersInternJobType] = currentJob.others[jobKeys.othersInternJobType]
    ...
    return <>
        <div className="current__Job__Display">
            ...
            <span className="display__Flex edit__Page__Font__Size">
                <b>Payment: </b> <input type={"text"} name={jobKeys.paymentForJob} value={currentJob.others && currentJob.others[jobKeys.paymentForJob]} onChange={(e) => updateJobDetails(prevValue => { return { ...prevValue, [jobKeys.others]: { ...prevValue["others"], [jobKeys.paymentForJob]: e.target.value } } })} />
            </span>
